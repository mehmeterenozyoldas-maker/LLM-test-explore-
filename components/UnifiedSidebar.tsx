...
import { BoxSelect, FastForward, Grab, History, Info, Loader2, MousePointer2, RotateCcw, Scan, Send, Settings2, Thermometer, X } from 'lucide-react';
import { useState } from 'react';
import { DetectedItem, DetectType, LogEntry } from '../types';

function LogOverlay({ log }: { log: LogEntry }) {
  const result = log.result as DetectedItem[] | null;
      ...
      {result.map((item, idx) => {
        if (item.box_2d) {
          ...
          return (
            <div
              key={idx}
              className="absolute border border-indigo-400 bg-indigo-400/20"
              style={{
                top: `${ymin / 10}%`,
                left: `${xmin / 10}%`,
                width: `${(xmax - xmin) / 10}%`,
        ...
        if (item.point) {
          ...
          return (
            <div
              key={idx}
              className="absolute w-1.5 h-1.5 bg-red-500 rounded-full border border-white transform -translate-x-1/2 -translate-y-1/2 shadow-sm"
              style={{
                top: `${y / 10}%`,
                left: `${x / 10}%`,
              }}
...
interface UnifiedSidebarProps {
  isOpen: boolean;
  onClose: () => void;
  onSend: (prompt: string, type: DetectType, temperature: number, enableThinking: boolean) => void;
  onPickup: () => void;
  isLoading: boolean;
    ...
    <div className={`absolute top-4 bottom-4 left-4 right-4 min-[660px]:left-auto min-[660px]:top-10 min-[660px]:right-10 min-[660px]:bottom-10 min-[660px]:w-96 glass-panel rounded-[2.5rem] flex flex-col z-40 overflow-hidden shadow-2xl transition-all border border-white/20 ${panelBase}`}>
      ...
      <div className={`p-8 border-b flex justify-between items-center ${headerBorder}`}>
        <div className="flex items-center gap-4">
          <div>
            <h2 className="text-lg font-bold leading-none">Embodied Reasoning</h2>
            <p className={`text-[10px] font-bold uppercase tracking-widest mt-1 ${isDarkMode ? 'text-indigo-400' : 'text-indigo-500'}`}>gemini-robotics-er-1.5-preview</p>
          </div>
        </div>
        <button onClick={onClose} className="p-2 hover:bg-slate-200/20 rounded-full transition-colors text-slate-400">
          <X className="w-5 h-5" />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar px-6 pt-1.5 pb-6 space-y-2">
          ...
          <div className={`p-1.5 rounded-2xl flex border ${selectorBg}`}>
              {(['2D bounding boxes', 'Segmentation masks', 'Points'] as DetectType[]).map((t) => (
                <button 
                  ...
                  className={`flex-1 py-3 rounded-xl flex flex-col items-center gap-1 transition-all ${type === t ? selectorActive : 'text-slate-500 hover:text-slate-400'}`}
                >
        ...
        <section className="space-y-2">
          ...
          <div className="flex gap-3">
            <button 
                onClick={() => onSend(prompt, type, temperature, enableThinking)}
                disabled={isLoading || !prompt.trim()}
                title="Detect: Trigger Gemini analysis of current workspace"
                className={`flex-1 py-4 rounded-2xl font-bold text-sm flex items-center justify-center gap-2 transition-all ${isLoading ? 'bg-slate-700/50 text-slate-500' : 'bg-slate-900 text-white hover:bg-black shadow-lg active:scale-[0.98] dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-white'}`}
            >
                      ...
                      {log.result === null ? (
                        ...
                        <div className="flex items-center justify-between mt-2 gap-2">
                           ...
                           <button 
                             onClick={(e) => {
                               e.stopPropagation();
                               onSend(log.prompt, log.type as DetectType, temperature, enableThinking);
                             }}
