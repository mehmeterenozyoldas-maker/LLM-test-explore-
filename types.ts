 ...
 * Axis Control Modes
 */
export type AxisMode = 'Up/Down' | 'Left/Right' | 'Close/Far' | 'Static';

/**
 * Control Mapping Configuration
 */
export interface ControlMap {
    left: { red: AxisMode; green: AxisMode; blue: AxisMode; };
    right: { red: AxisMode; green: AxisMode; blue: AxisMode; };
}
...
export interface MujocoModule {
  ...
  mj_name2id: (m: MujocoModel, type: number, name: string) => number;
  mjtGeom: Record<string, number | {value: number}>;
  mjtObj: Record<string, number | {value: number}>;
  FS: {
